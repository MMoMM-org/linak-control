...
_ih_render() {
    ...
    local dst="$2"
    cp "$src" "$dst"
    REPLACE_VAL="$REPO_NAME"       perl -i -pe 's/\Qlinak-control\E/$ENV{REPLACE_VAL}/g'       "$dst"
    REPLACE_VAL="$REPO_MOUNT_PATH" perl -i -pe 's/\Q/Volumes/Moon/Coding/linak-control\E/$ENV{REPLACE_VAL}/g' "$dst"
