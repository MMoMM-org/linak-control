...
<!-- 2026-08-31 -->
## Test suite wrote into the real debug.log -- Status: resolved (issue #18)
`FileLog.logURL` resolved from `homeDirectoryForCurrentUser` at load time with no override, and tests use `@testable import`, so every test touching DeskManager/handshake/status logged into `~/Library/Logs/LinakControl/debug.log`. Measured: 5 tests = 13KB; a full suite run leaves fixture heights (730/1103/1105), rapid-fire `executeGoToPreset`, and FABRICATED E16/E26 `[status]` lines that never happened on hardware -- in the exact file `docs/troubleshooting.md` tells users to send. **When reading any log slice dated before 2026-08-31, check for these before drawing conclusions.** Fix: `guard !isRunningUnderTest else { return nil }` in `logURL`; `append` already guards on nil. Detection is `NSClassFromString("XCTestCase") != nil` -- the app target does not link XCTest. IMPORTANT: `XCTestConfigurationFilePath` is NOT set under `swift test` (measured), so do not reach for that env var.
